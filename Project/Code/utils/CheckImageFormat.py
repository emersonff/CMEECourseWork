#!usr/bin/env python3
"""get filnames of corrput or broken JPG iamges that can not be processed using the Openpose library"""
__appname__ = "Check images` format"
...
### globals
f = 'd_C2/'#change here accordingly. folder that hold the images.
...
def main(argv):
    ...
    for i, filename in enumerate(os.listdir(f)):
        with open(f + filename, 'rb') as imageFile:
            flag = 0
            if not imageFile.read().startswith(b'\xff\xd8'):#a JPG iamge shold be started with
                print(f"{i}: {filename} - found!")          #b'\xff\xd8' and end with  b'\xff\xd9'.
                flag = 1
